...
async function buscarDocUsuario(uid, email = null) {
  // Intentar por perfil.uid (campo real en Firestore)
  let q = query(collection(db, 'usuarios'), where('perfil.uid', '==', uid), limit(1));
  let snap = await getDocs(q);

  // Fallback: campo uid en raíz (por si algunos docs lo tienen así)
  if (snap.empty) {
    q = query(collection(db, 'usuarios'), where('uid', '==', uid), limit(1));
    snap = await getDocs(q);
  }

  // Fallback: buscar por perfil.email si se provee
  if (snap.empty && email) {
    q = query(collection(db, 'usuarios'), where('perfil.email', '==', email), limit(1));
    snap = await getDocs(q);
  }

  // Fallback: email en raíz
  if (snap.empty && email) {
    q = query(collection(db, 'usuarios'), where('email', '==', email), limit(1));
    snap = await getDocs(q);
  }
...
// ─── Operar directo por docId (para admin, sin re-búsqueda) ──────────────────
async function resolverPorDocId(docId) {
  const docRef  = doc(db, 'usuarios', docId);
  const docSnap = await getDoc(docRef);
  if (!docSnap.exists()) throw new Error('Usuario no encontrado');
  return { docRef, docId, data: docSnap.data() };
}
...
// ─── Agregar / restar puntos por docId (admin) ───────────────────────────────
//
// Evita buscarDocUsuario cuando ya tenemos el docId del documento.
// Usar desde PanelAdmin pasando usuarioFound.id directamente.
//
export async function agregarPuntosConDocId(docId, accion, opciones = {}) {
  const accionInfo = ACCIONES[accion];
  if (!accionInfo) throw new Error(`Acción desconocida: ${accion}`);

  const { docRef, data: userData } = await resolverPorDocId(docId);
  ...
  batch.set(txRef, {
    ...
    metadata: {
      descripcion:  opciones.descripcion || accionInfo.label,
      aprobado_por: opciones.aprobado_por || 'admin',
      monto:        opciones.monto        || null,
    },
  });
  await batch.commit();
...
export async function restarPuntosConDocId(docId, puntosARestar, motivo, opciones = {}) {
  const { docRef, data: userData } = await resolverPorDocId(docId);
